...
class DividerExample {
    ...
    @Test
    fun `should extract the data from the table with dividers where some are not allowed`() {
        // given
        val headerColumns = setOf(
            StringHeaderCell("team"),
            StringHeaderCell("plays"),
        ...
        val definition = WorkbookParserDefinition(
            spreadsheetParserDefinitions = listOf(
                SheetParserDefinition(
                    sheetNameFilter = { it == "table with constrained dividers" },
                    tableDefinitions = listOf(
                        TableParserDefinition(
                            requiredColumns = headerColumns,
                            hasDivider = true,
        ...
        assertThat(parsedRecords)
            ...
            .containsExactly(
                ...
                GenericParsedRecord(
                    mapOf(
                        "workbook_name" to fileName,
                        "spreadsheet_name" to "table with constrained dividers",
                        "row_number" to 3,
                        "divider" to "Group A",
                        "team" to "PSG",
                        "plays" to 3,
                ...
                // data after the divider we don't want "Group that is to be ignored as a divider"
                GenericParsedRecord(
                    mapOf(
                        "workbook_name" to fileName,
                        "spreadsheet_name" to "table with constrained dividers",
